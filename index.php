<?php
   ...
   global $dbCon;
   $dbCon = connectToDB("root","root","localhost","labsdb");
...
//      $r = process("select id, description from categories where id regexp concat(".mysqlReady(str_replace('.','\\.','^'.$pos)).",'\\.') order by datetime desc");
   ...
   function reviewQuestions() {
      ...
      echo "Page: 1 2 3 ... Next Prev Go to:[] &nbsp;&nbsp;&nbsp;&nbsp;<select name='filter'>
         <option value=''>Filter Questions</option>
         <option value='All'>Show All Questions</option>
         <option value='byUser'>Show Questions by a specific user...</option>
         <option value='tagged'>Show Tagged Questions</option>
         </select><br />";      
      $r=process("select title, question, owner, privileges, id from questions where subcategory regexp ('$pos' | concat(".mysqlReady(str_replace('.','\\.','^'.$pos)).",'\\.')) order by id desc");
      echo("select title, question, owner, privileges, id from questions where subcategory regexp ('$pos' | concat(".mysqlReady(str_replace('.','\\.','^'.$pos)).",'\\.')) order by id desc");
      print_r($r);
      echo "<br />Delete  Edit Tag<br />";
      echo 123; exit;
      // prompt for question?
      if (!isset($_REQUEST['label'])) {
         echo
...
echo "<br />"; print_r($_SESSION);
      ...
      exit;
      // get next position 
      $r = process("select max(count) from categories where id  concat(".mysqlReady(str_replace('.','\\.','^'.$pos)).",'\\.[0-9]+$')");
      if (is_array($r) && ($r[0][0] > 0)) {
         $r = process("select id from categories where count=".$r[0][0]);
         $list = explode(".",$r[0][0]);
         $next = "$pos.".($list[count($list)-1]+1);
